...
{
	TVPThrowExceptionMessage(TJS_W("PNG Read Error /%1"), msg);
}
//---------------------------------------------------------------------------
// user_warning_fn
...
{
	ULONG read;
	((IStream *)png_get_io_ptr(png_ptr))->Read((void*)data, length, &read);
	// ((IStream *)png_get_io_ptr(png_ptr))->ReadBuffer((void*)data, (tjs_uint)length);
}
//---------------------------------------------------------------------------
// read_row_callback
...
static int PNG_read_chunk_callback(png_structp png_ptr,png_unknown_chunkp chunk)
{
		...
		(chunk->name[3] == 0x67/*'g'*/ || chunk->name[3] == 0x47/*'G'*/) && chunk->size >= 9)
	{
		...
		*/
		...
		user_struct->metainfopushcallback(user_struct->callbackdata, ttstr(TJS_W("vpag_w")), ttstr((tjs_int)width));
		user_struct->metainfopushcallback(user_struct->callbackdata, ttstr(TJS_W("vpag_h")), ttstr((tjs_int)height));
		switch(unit)
		{
		case PNG_OFFSET_PIXEL:
			user_struct->metainfopushcallback(user_struct->callbackdata, ttstr(TJS_W("vpag_unit")), ttstr(TJS_W("pixel")));
			break;
		case PNG_OFFSET_MICROMETER:
			user_struct->metainfopushcallback(user_struct->callbackdata, ttstr(TJS_W("vpag_unit")), ttstr(TJS_W("micrometer")));
			break;
		default:
			user_struct->metainfopushcallback(user_struct->callbackdata, ttstr(TJS_W("vpag_unit")), ttstr(TJS_W("unknown")));
			break;
		}
...
void TVPLoadPNG(void* formatdata, void *callbackdata, tTVPGraphicSizeCallback sizecallback, tTVPGraphicScanLineCallback scanlinecallback, tTVPMetaInfoPushCallback metainfopushcallback, IStream *src, tjs_int keyidx, tTVPGraphicLoadMode mode)
{
	...
	if( !png_ptr ) TVPThrowExceptionMessage(TJS_W("PNG Read Error /%1"), TJS_W("libpng error.") );
	...
	if( !info_ptr ) TVPThrowExceptionMessage(TJS_W("PNG Read Error /%1"), TJS_W("libpng error.") );

	// create end_info
	end_info=png_create_info_struct(png_ptr);
	if( !end_info ) TVPThrowExceptionMessage(TJS_W("PNG Read Error /%1"), TJS_W("libpng error.") );
		...
		png_get_oFFs(png_ptr, info_ptr, &offset_x, &offset_y, &offset_unit_type))
	{
		...
		metainfopushcallback(callbackdata, ttstr(TJS_W("offs_x")), ttstr((tjs_int)offset_x));
		metainfopushcallback(callbackdata, ttstr(TJS_W("offs_y")), ttstr((tjs_int)offset_y));
		switch(offset_unit_type)
		{
		case PNG_OFFSET_PIXEL:
			metainfopushcallback(callbackdata, ttstr(TJS_W("offs_unit")), ttstr(TJS_W("pixel")));
			break;
		case PNG_OFFSET_MICROMETER:
			metainfopushcallback(callbackdata, ttstr(TJS_W("offs_unit")), ttstr(TJS_W("micrometer")));
			break;
		default:
			metainfopushcallback(callbackdata, ttstr(TJS_W("offs_unit")), ttstr(TJS_W("unknown")));
			break;
		}
	}
	...
	{
		// push offset information into metainfo data
		metainfopushcallback(callbackdata, ttstr(TJS_W("reso_x")), ttstr((tjs_int)reso_x));
		metainfopushcallback(callbackdata, ttstr(TJS_W("reso_y")), ttstr((tjs_int)reso_y));
		switch(reso_unit_type)
		{
		case PNG_RESOLUTION_METER:
			metainfopushcallback(callbackdata, ttstr(TJS_W("reso_unit")), ttstr(TJS_W("meter")));
			break;
		default:
			metainfopushcallback(callbackdata, ttstr(TJS_W("reso_unit")), ttstr(TJS_W("unknown")));
			break;
		}
	}
	...
	if(mode == glmPalettized)
	{
		...
		if(bit_depth > 8)
			TVPThrowExceptionMessage(
				TJS_W("PNG Read Error /%1"), TJS_W("Unsupported color type for palattized image") );
	...
	else
	{
		...
		default:
			TVPThrowExceptionMessage(
					TJS_W("PNG Read Error /%1"), TJS_W("Unsupported color type") );
		}
	}
	...
	if(width>=65536 || height>=65536)
	{
		...
		TVPThrowExceptionMessage(
			TJS_W("PNG Read Error /%1"), TJS_W("Too large image") );
	}
...
static void PNG_write_write( png_structp png_ptr, png_bytep buf, png_size_t size )
{
    ...
    // dst
    ((IStream *)png_get_io_ptr(png_ptr))->Write(buf, size, NULL);
}
//---------------------------------------------------------------------------
/**
 * PNG書き込み
...
bool TVPAcceptSaveAsPNG(void* formatdata, const ttstr & type, class iTJSDispatch2** dic ) {
	bool result = false;
	if( type.StartsWith(TJS_W("png")) || (type == TJS_W(".png")) ) result = true;
	if( result && dic ) *dic = TJSCreateDictionaryObject();
	return result;
}

void TVPSaveAsPNG(void* formatdata, void* callbackdata, IStream* dst, const ttstr & mode, tjs_uint width, tjs_uint height, tTVPGraphicSaveScanLineCallback scanlinecallback, iTJSDispatch2* meta ) {
	int bpp = 32;
	...
	// if( width == 0 || height == 0 ) TVPThrowInternalError;
	...
	png_set_write_fn( png_ptr, (png_voidp)dst, (png_rw_ptr)PNG_write_write, (png_flush_ptr)PNG_write_flash );
	...
	if( bpp == 32 ) {
		for( tjs_uint32 y = 0; y < height; y++ ) {
			memcpy( buff, scanlinecallback(callbackdata,y), width_byte );
			png_write_row( png_ptr, (png_bytep)buff );
		}
	} else {
		for( tjs_uint32 y = 0; y < height; y++ ) {
			const tjs_uint8* src = reinterpret_cast<const tjs_uint8*>(scanlinecallback(callbackdata,y));
			tjs_uint8* dst = buff;
...
void TVPLoadHeaderPNG(void* formatdata, IStream *src, iTJSDispatch2** dic) {
	png_structp png_ptr=NULL;
	png_infop info_ptr=NULL;
	png_infop end_info=NULL;
